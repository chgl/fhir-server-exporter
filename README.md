...
docker run --rm -it \
    ...
    ghcr.io/chgl/fhir-server-exporter:v3.0.19
```
...
- docker.io/chgl/fhir-server-exporter:v3.0.19
- ghcr.io/chgl/fhir-server-exporter:v3.0.19
...
| FhirLakehouse\_\_S3\_\_Endpoint | The S3 endpoint, e.g. `localhost:8333`. Credentials must be configured using the `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` environment variables.                   | `""`             |
...
```sh
IMAGE=ghcr.io/chgl/fhir-server-exporter:v3.0.19
